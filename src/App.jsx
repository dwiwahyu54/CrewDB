...
export default function App() {
  ...
  const rankOptions = useMemo(()=>{
    ...
    crews.forEach((c)=>{
      ...
      c.coe.forEach((x)=>x.name&&s.add(x.name));
    });
    return [...s].sort();
  },[crews]);
    ...
    return crews.filter((c)=>{
      ...
      if (filterRank) {
        const has=c.experience.some((x)=>x.rank===filterRank)
               ||c.coc.concat(c.coe).some((x)=>x.name===filterRank);
        if (!has) return false;
      }
...
// For Vercel deployment: set VITE_ANTHROPIC_API_KEY in your Vercel environment variables
// The fetch in parseViaAI above works automatically in the artifact preview
// For your own deployment, the API call goes through Anthropic's API directly from the browser
